...
*The R markdown is available from the pulldown menu for* Code *at the upper-right, choose "Download Rmd", or [download the Rmd from GitHub](https://github.com/jpinero/disgenet-app-notebook/DisGeNET_App.Rmd).*

<hr />

This tutorial will teach you how to use the [DisGeNET Cytoscape App](http://apps.cytoscape.org/apps/disgenetapp) to retrieve data from [DisGeNET](https://www.disgenet.org/) [1].


**DisGeNET**  

DisGeNET is a discovery platform containing one of the largest publicly available collections of genes and variants associated with human diseases [1]. DisGeNET integrates data from expert curated repositories, GWAS catalogues, animal models and the scientific literature. The data in the platform are homogeneously annotated with controlled vocabularies and community-driven ontologies. Additionally, several original metrics are provided to assist the prioritization of genotype–phenotype relationships.

To complete the tutorial you will need to install **RCy3**:
...
Finally, install the DisGENET App to access DisGeNET from within Cytoscape. This can be done from http://apps.cytoscape.org/apps/disgenetapp or, if you are running Cytoscape 3.7.0 or above, by simply running this command:
...
In these exercises, we will learn how to use the [DisGeNET Cytoscape App](http://apps.cytoscape.org/apps/disgenetapp) to:
...
* Perform a variant enrichment analyses
* Create a GDA network associated to a list of genes   
* Perform a gene enrichment analysis

First, we need to install and load the library *httr*
...
Then, we will create an object for the REST calls to the DisGeNET automation module.  
The parameters are:  
...
In this example, we will create a gene-disease network for Asthma using the CURATED data in DisGeNET
Below, an example of the object *netParams* to create the GDA network 
...
variants <-read.csv("data/variants.tsv")
variants <- variants$x
...
- *universe*: universe for the Fisher test. We provide 3 variants sets to be used as universe:  
  
DISGENET - All variants in DisGeNET  
HUMAN - All variants according to CLINVAR (pathogenic/non-pathogenic variants), UNIPROT (disease variants and polymorphisms), all variants from GWASCAT and GWASDB without filtering by p-value  

If no universe is supplied, by default the function will use all the variants in DisGeNET.
...
edges <-read.csv("data/variant_gene_network.tsv",sep = "\t")
...
- *source*: the source database (by default, `database = “CURATED”`)
- *universe*: universe for the Fisher test. We provide 3 gene sets to be used as universe:  
...
Now, you can create a GDA network for the list of genes of interest and the top diseases from the enrichment.


```{r}
geneDisParams <- list(
  source = "CURATED",
  geneSearch = paste(unique(edges$target), collapse = ";"),
  diseaseSearch= paste(unique(top_nodes$diseaseId), collapse = ";"),
  initialScoreValue = "0.4",
  finalScoreValue = "1.0"
)
